...
class Driver {
  ...
  async publisherPerform(args) {
    await this.bus.publish(args.bus_event_type, args, function(err){
      callback(err, self.options.busClassKey);
    });
  };

  async driverPerform(args) {
    let started = 0;
    let matches = [];

    let end = function (error) { console.log(error) }

    this.bus.subscriptions(function(err, subscriptions, count){
      if(err){
        end(err);
      }else if(count === 0){
        end();
...
heartbeatPerform(args, callback) {
  var self = this;
...
driverMetadata(args, bus_rider_queue, bus_rider_app_key, bus_rider_sub_key, bus_rider_class_name, bus_event_type) {
  var payload = {};
...
subscriptionMatch(args, subscription) {
