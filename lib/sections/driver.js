  ...
  return {
    perform: function(args, callback){
      ...
      if(args.bus_class_proxy === '::QueueBus::Driver'){   
        driverPerform(self, args, callback); 
      }else if(args.bus_class_proxy === '::QueueBus::Rider'){    
        callback(new Error('TODO: ::QueueBus::Rider')); // TODO!
      }else if(args.bus_class_proxy === '::QueueBus::Publisher'){
        publisherPerform(self, args, callback); 
      }else if(args.bus_class_proxy === '::QueueBus::Heartbeat'){
        callback(new Error('TODO: ::QueueBus::Heartbeat')); // TODO!
      }else{
        callback(new Error('Missing bus_class_proxy or unknown method (' + args.bus_class_proxy + ')'));
      }
    } 
  };
};

var publisherPerform = function(self, args, callback){
  self.publish(args.bus_event_type, args, function(err){
    callback(err, self.options.busClassKey);
  });
};

var driverPerform = function(self, args, callback){
  var started = 0;
  var matches = [];
      ...
      for(var app in subscriptions){
        for(var i in subscriptions[app]){
          ...
          if(matched === true){
            ...
            payload.bus_class_proxy = '::QueueBus::Rider';
