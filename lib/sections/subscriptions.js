...
class Subscriptions {
  ...
  async subscriptions () {
    ...
    for (let app in applications) {
      started++;
      console.log(`prefix: ${this.fullAppPrefix(this) + app}`);
      let rawSubscriptions = await this.connection.redis.hgetall(this.fullAppPrefix(this) + app)
      console.log(`this rawSubscriptions: ${JSON.stringify(rawSubscriptions)}`);
      for(let i in rawSubscriptions){
        if(subscriptions[app] === null || subscriptions[app] === undefined){ subscriptions[app] = {}; }
        console.log(`i: ${i} subscriptions ${subscriptions} subscription ${JSON.parse(rawSubscriptions[i])}`)
  ...
  async subscribe(appKey, priority, job, matcher){
    ...
    console.log(`subscribe sadd queueName: ${combined_queue_name} result: ${bresult}`);
