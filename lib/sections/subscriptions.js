...
class Subscriptions {
  ...
  async subscriptions () {
    ...
    let applications = await this.bus.connection.redis.smembers(this.fullSubscriptionSet(this));
    ...
    for (let app of applications) {
      ...
      let rawSubscriptions = await this.bus.connection.redis.hgetall(this.fullAppPrefix(this) + app)
  ...
  async subscribe(appKey, priority, job, matcher){
    ...
    let result = await this.bus.connection.redis.hset(this.fullAppPrefix(this) + appKey, key, JSON.stringify(subscription))
    ...
    let bresult = await this.bus.connection.redis.sadd(this.fullSubscriptionSet(this), appKey);
  ...
  async unsubscribe(appKey, priority, job){
    appKey      = utils.normalize(appKey);
    let key     = utils.hashKey(appKey, priority, job);   
    await this.bus.connection.redis.hdel(this.fullAppPrefix(this) + appKey, key);
    let keys = await this.bus.connection.redis.hkeys(this.fullAppPrefix(this) + appKey);
  ...
  async unsubscribeAll(appKey){
    appKey   = utils.normalize(appKey);
    await this.bus.connection.redis.srem(this.fullSubscriptionSet(this), appKey);
    await this.bus.connection.redis.del(this.fullAppPrefix(this) + appKey);
  };
